...
#define SWVer "06.01"                                   // Major.Minor Software version (use String 01.00 - 99.99 format !)
...
void config_defaults() {
    ...
    strcpy(config.Location, "Casa");                      // Device Location (usar Casa)
    ...
    config.Temp_Corr = 0.0F;                              // Sensor Temperature Correction Factor, typically due to electronic self heat.
    config.LDO_Corr = 0.1F;                               // Battery Voltage [volt] corrective Factor due to LDO/Diode voltage drop
}
